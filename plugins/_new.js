...
cmd({
  pattern: "news",
  alias: "actualités",
  desc: "Affiche les dernières actualités avec images.",
  react: '📰',
  category: "info",
  filename: __filename
}, async (_0x5351f6, _0x1439a7, _0x278458, { reply }) => {
  try {
    let apiKey = "5dadec43f17e4cec85f2b853dbc787b7"; // Remplace avec ta clé API News
    let url = `https://newsapi.org/v2/top-headlines?country=fr&apiKey=${apiKey}`;

    let response = await axios.get(url);
    let articles = response.data.articles.slice(0, 5); // Récupère les 5 premières actualités

    for (let article of articles) {
      let message = `📰 *${article.title}*\n\n📌 *Source* : ${article.source.name}\n📅 *Publié le* : ${new Date(article.publishedAt).toLocaleDateString('fr-FR')}\n\n🔗 *Lien* : ${article.url}`;

      await _0x5351f6.sendMessage(_0x1439a7, {
        image: { url: article.urlToImage || "https://via.placeholder.com/500" }, // Image de l'article ou image par défaut
        caption: message
      });
    }
  } catch (error) {
    console.log(error);
    await reply("❌ Erreur lors de la récupération des actualités.");
  }
});
cmd({
  pattern: "weather",
  ...
  category: "info",
  filename: __filename
}, async (_0x5351f6, _0x1439a7, _0x278458, { args, reply }) => {
  try {
    if (!args[0]) return reply("❌ Veuillez spécifier une ville (ex: weather Paris)");

    let city = args.join(" ");
    let apiKey = "3605a04df5ff03a13c78006f26e5c326"; // Remplace avec ta clé API météo
    ...
    console.log(error);
    await reply("❌ Erreur lors de la récupération de la météo.");
  }
});
cmd({
  pattern: "crypto",
  alias: "bitcoin",
  desc: "Affiche le prix d'une cryptomonnaie.",
  react: '📈',
  category: "info",
  filename: __filename
}, async (_0x5351f6, _0x1439a7, _0x278458, { args, reply }) => {
  try {
    if (!args[0]) return reply("❌ Veuillez spécifier une cryptomonnaie (ex: crypto bitcoin)");
    ...
    let message = `💰 *Prix du ${data.name} (${data.symbol.toUpperCase()})*\n\n📊 *Prix actuel* : ${data.market_data.current_price.usd} USD\n📈 *Haut 24h* : ${data.market_data.high_24h.usd} USD\n📉 *Bas 24h* : ${data.market_data.low_24h.usd} USD`;
    ...
    console.log(error);
    await reply("❌ Cryptomonnaie introuvable.");
  }
});
cmd({
  pattern: "quote",
  ...
  try {
    ...
    await _0x5351f6.sendMessage(_0x1439a7, {
      image: { url: "https://source.unsplash.com/500x500/?motivation,success" }, // Image aléatoire de motivation
      caption: message
    });
  } catch (error) {
    console.log(error);
    await reply("❌ Erreur lors de la récupération de la citation.");
  }
});
cmd({
  pattern: "anime",
  alias: "manga",
  desc: "Affiche des infos sur un anime.",
  react: '🎌',
  category: "fun",
  filename: __filename
}, async (_0x5351f6, _0x1439a7, _0x278458, { args, reply }) => {
  try {
    if (!args[0]) return reply("❌ Veuillez spécifier un anime (ex: anime Naruto)");

    let animeName = args.join(" ");
    let url = `https://api.jikan.moe/v4/anime?q=${animeName}&limit=1`;

    let response = await axios.get(url);
    let data = response.data.data[0];

    let message = `🎌 *${data.title}*\n\n📅 *Sortie* : ${data.aired.string}\n🎭 *Genres* : ${data.genres.map(g => g.name).join(", ")}\n⭐ *Note* : ${data.score}/10\n📜 *Synopsis* : ${data.synopsis}`;
    ...
    console.log(error);
    await reply("❌ Anime introuvable.");
  }
});
cmd({
  pattern: "randomfact",
  alias: "fait",
  desc: "Affiche un fait étonnant.",
  react: '🤯',
  category: "fun",
  filename: __filename
}, async (_0x5351f6, _0x1439a7, _0x278458, { reply }) => {
  try {
    let url = "https://uselessfacts.jsph.pl/random.json?language=fr";

    let response = await axios.get(url);
    let message = `🤯 *Fait surprenant !*\n\n${response.data.text}`;

    await _0x5351f6.sendMessage(_0x1439a7, {
      image: { url: "https://source.unsplash.com/500x500/?wow" }, // Image aléatoire sur l’étonnement
      caption: message
    });
  } catch (error) {
    console.log(error);
    await reply("❌ Erreur lors de la récupération du fait surprenant.");
  }
});
cmd({
  pattern: "history",
  alias: "histoire",
  desc: "Affiche un événement historique du jour.",
  react: '📜',
  category: "culture",
  filename: __filename
}, async (_0x5351f6, _0x1439a7, _0x278458, { reply }) => {
  try {
    ...
    let event = events[Math.floor(Math.random() * events.length)]; // Prend un événement aléatoire du jour

    let message = `📜 *Événement du ${response.data.date}*\n\n📅 *Année* : ${event.year}\n📖 *Détails* : ${event.text}\n🔗 *Plus d’infos* : ${event.links[0].link}`;

    await _0x5351f6.sendMessage(_0x1439a7, {
      image: { url: "https://source.unsplash.com/500x500/?history" }, // Image générique sur l’histoire
      caption: message
    });
  } catch (error) {
    console.log(error);
    await reply("❌ Erreur lors de la récupération de l'événement historique.");
  }
});
cmd({
  pattern: "football",
  alias: "foot",
  desc: "Affiche les derniers résultats de football.",
  react: '⚽',
  category: "sports",
  filename: __filename
}, async (_0x5351f6, _0x1439a7, _0x278458, { reply }) => {
  try {
    let apiKey = "f2aec2ced314aaf4a10b905367679566"; // Remplace avec ta clé API Football
    let url = `https://api.football-data.org/v4/matches?status=LIVE&apikey=${apiKey}`;

    let response = await axios.get(url, { headers: { "X-Auth-Token": apiKey } });
    let matches = response.data.matches.slice(0, 3); // Affiche 3 matchs en direct

    if (matches.length === 0) return reply("⚽ Aucun match en direct actuellement.");

    for (let match of matches) {
      let home = match.homeTeam.name;
      let away = match.awayTeam.name;
      let score = `${match.score.fullTime.home} - ${match.score.fullTime.away}`;

      let message = `⚽ *Match en direct*\n🏆 *Compétition* : ${match.competition.name}\n⚔️ *${home} vs ${away}*\n📊 *Score* : ${score}`;

      await _0x5351f6.sendMessage(_0x1439a7, {
        image: { url: match.competition.emblem || "https://via.placeholder.com/500" },
        caption: message
      });
    }
  } catch (error) {
    console.log(error);
    await reply("❌ Erreur lors de la récupération des matchs.");
  }
});
 smd({
   cmdname: "listmessage",
 ...
 smd({
   ...
   try {
     let _0x12a593 = "*JINHUYK-MD CURRENTLY ACTIVE BRO🍁!*";
 ...
 smd({
   ...
   try {
     ...
     if (!_0x587b99) {
       return await _0x4cdec8.reply("*Need URL! Use " + prefix + "ss https://github.com/ChanJinhuyk/JINHUYK-MD-V1*");
     }
 ...
 let tmpUrl = "https://i.postimg.cc/sx2KY0mS/JINHUYK-MD-V1.jpg";
 const secmailData = {};
 smd({
   pattern: "tempmail",
