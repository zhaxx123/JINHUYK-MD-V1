    ...
    try {
      ...
      const designs = [
        ...
        async () => {
          ...
          const end = new Date().getTime();
          const pingSeconds = (end - start) / 1000;
          const captionText = `JINHYUK-MD\n\n*Ping:* ${pingSeconds} seconds\n\n*Fact:*\n${fact.result.fact}\n\n+JINHYUK-MD`;

          return { image: imageBuffer.data, caption: captionText };
        },
        async () => {
          const imageBuffer = await axios.get(
            "https://files.catbox.moe/jynitk.jpg",
            {
              responseType: "arraybuffer",
            }
          );
...
smd(
  ...
  async (m) => {
    try {
      ...
      const message = `*ᵏᵍ┘JINHYUK-MD└ ʀᴜɴɴɪɴɢ sɪɴᴄᴇ:* ${uptimeHours}h ${uptimeMinutes}ᴍ ${uptimeSeconds}s\n \t_ʜᴇʀᴇ's ᴍᴏʀᴇ ɪɴғᴏ_\n*ʀᴀᴍ ᴜsᴀɢᴇ:* ${ramUsage.toFixed(2)} MB\n*ᴄᴘᴜ ɴᴀᴍᴇ:* ${cpuModel}`;
      const contextInfo = {
        isForwarded: true,
        forwardingScore: 999,
        title: "ᵏᵍ┘JINHYUK-MD└ Running Since",
        body: message,
        footerText: "ᵏᵍ┘JINHYUK-MD└ 2024",
